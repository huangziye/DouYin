...
class Love : RelativeLayout {
    ...
    // 动画中随机❤的旋转角度
    private var mAngle = floatArrayOf(-35f, -25f, 0f, 25f, 35f)
    ////用来判断是否是连续的点击事件
    ...
    override fun onTouchEvent(event: MotionEvent?): Boolean {
        ...
        if (mHits[0] >= (SystemClock.uptimeMillis() - 500)) {
            ...
            animatorSet.play(
                //缩放动画，X轴2倍缩小至0.9倍
                scaleAnimation(iv, "scaleX", 2f, 0.9f, 100, 0)
            )
                //缩放动画，Y轴2倍缩放至0.9倍
                .with(scaleAnimation(iv, "scaleY", 2f, 0.9f, 100, 0))
                //旋转动画，随机旋转角
                .with(rotation(iv, 0, 0, mAngle[Random().nextInt(4)]))
                //渐变透明动画，透明度从0-1
                .with(alphaAnimation(iv, 0F, 1F, 100, 0))
                //缩放动画，X轴0.9倍缩小至
                .with(scaleAnimation(iv, "scaleX", 0.9f, 1F, 50, 150))
                //缩放动画，Y轴0.9倍缩放至
                .with(scaleAnimation(iv, "scaleY", 0.9f, 1F, 50, 150))
                //位移动画，Y轴从0上移至600
                .with(translationY(iv, 0F, -600F, 800, 400))
                //透明动画，从1-0
                .with(alphaAnimation(iv, 1F, 0F, 300, 400))
                //缩放动画，X轴1至3倍
                .with(scaleAnimation(iv, "scaleX", 1F, 3f, 700, 400))
                //缩放动画，Y轴1至3倍
                .with(scaleAnimation(iv, "scaleY", 1F, 3f, 700, 400))
    ...
    fun alphaAnimation(view: View, from: Float, to: Float, time: Long, delayTime: Long): ObjectAnimator {
